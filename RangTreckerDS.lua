...
function sendDiscord(buyer, rank, days, price, profit, title)
    ...
    local color = 3447003
    if tostring(title):find("\xD0\x9F\xD0\xA0\xD0\x9E\xD0\x94") or tostring(title):find("опндкемхе") then color = 3066993 end
...
function sendLog(buyer, rank, days, price, profit, title)
    ...
    addLogLine(string.format("[%s] %s | %s | %s ДМ. | %s",
...
function sampev.onShowDialog(id, style, title, button1, button2, text)
    pcall(function()
        ...
        if not clean:find("нАЫЮЪ ЯРНХЛНЯРЭ") then return end
        local buyer = clean:match("[бБ]ШАПЮММШИ%s+ХЦПНЙ:%s*([%w_%-]+)%s*%(%d+%)")
                   or clean:match("([%w_%-]+)%(%d+%)")
        local rank  = clean:match("[бБ]ШАПЮММШИ%s+ПЮМЦ:%s*(.-)%s*%(%d+%)")
                   or clean:match("[пП]ЮМЦ%s+ХЦПНЙЮ:%s*(.-)%s*%(%d+%)")
        if rank then rank = rank:gsub("[%.%s]+$",""):match("^%s*(.-)%s*$") end
        local days       = clean:match("[бБ]ШАПЮММНЕ%s+ЙНК%-БН%s+ДМЕИ:%s*(%d+)")
        local total_str  = clean:match("[нН]АЫЮЪ%s+ЯРНХЛНЯРЭ:%s*(.-)%s*[\n%.]")
                        or clean:match("[нН]АЫЮЪ%s+ЯРНХЛНЯРЭ:%s*([^\n]+)")
        local profit_str = clean:match("[бБ]ЮЬ%s+ОПНЖЕМР:%s*(.-)%s*%(")
                        or clean:match("[бБ]ЮЬ%s+ОПНЖЕМР:%s*([^\n%(]+)")
...
function sampev.onServerMessage(color, text)
    pcall(function()
        local clean = stripColors(text)
        if clean:find("бШ ОПЕДКНФХКХ ХЦПНЙС") and clean:find("ПЮМЦ Б НПЦЮМХГЮЖХХ") then
            local pb, pr = clean:match("бШ ОПЕДКНФХКХ ХЦПНЙС ([%w_]+) .+ ПЮМЦ Б НПЦЮМХГЮЖХХ (.+)")
            if pb then Cache.buyer=pb; Cache.rank=pr or Cache.rank; Cache.time=os.time() end
        end
        if clean:find("ОПХМЪК ОНЙСОЙС ПЮМЦЮ") or clean:find("ОПХМЪК ОПНДКЕМХЕ ПЮМЦЮ") then
            local cb = clean:match("хЦПНЙ ([%w_]+)%(%d+%) ОПХМЪК") or clean:match("хЦПНЙ ([%w_]+) ОПХМЪК")
            local ps = clean:match("ПЮМЦЮ ГЮ (.+)$")
            if cb and ps then
                ps = ps:gsub("[%.$]",""):gsub("%s+","")
                ...
                local is_renewal = clean:find("ОПНДКЕМХЕ") ~= nil
                local op_title   = is_renewal and u8("опндкемхе пюмцю") or u8("онйсойю пюмцю")
...
-- ================= релю =================
local C = {}  -- ГЮОНКМЪЕРЯЪ Б OnInitialize
...
imgui.OnInitialize(function()
    ...
    -- рЕЙЯРНБШЕ ЖБЕРЮ (РНКЭЙН ОНЯКЕ ХМХЖХЮКХГЮЖХХ imgui)
...
local L = {
    tab_main     = u8("  цКЮБМЮЪ  "),
    tab_log      = u8("  кНЦ  "),
    tab_stats    = u8("  яРЮРХЯРХЙЮ  "),
    tab_settings = u8("  мЮЯРПНИЙХ  "),
    manager_lbl  = u8("лЕМЕДФЕП:"),
    last_entry   = u8("оНЯКЕДМЪЪ ГЮОХЯЭ:"),
    curr_lbl     = u8("рЕЙСЫЕЕ:"),
    name_lbl     = u8("хЛЪ ЛЕМЕДФЕПЮ:"),
    hint_input   = u8("мНБНЕ ХЛЪ:"),
    plat_lbl     = u8("оКЮРТНПЛЮ:"),
    ver_lbl      = u8("бЕПЯХЪ:"),
    pct_lbl      = u8("дНУНД %:"),
    dpi_lbl      = u8("лЮЯЬРЮА DPI:"),
    cmd_lbl      = u8("йНЛЮМДШ:"),
    days_sfx     = u8("ДМ."),
    no_data      = "\xE2\x80\x94",
    log_empty    = u8("кНЦ ОСЯР"),
    plat_mob     = "MonetLoader (Mobile)",
    plat_pc      = "MoonLoader (PC)",
    btn_send     = u8("нРОПЮБХРЭ Б Discord"),
    btn_test     = u8("рЕЯР"),
    btn_update   = u8("нАМНБХРЭ ЯЙПХОР"),
    btn_save     = u8("яНУПЮМХРЭ"),
    btn_reload   = u8("нАМНБХРЭ КНЦ"),
    btn_reset_dpi= u8("яАПНЯХРЭ DPI"),
    btn_copy     = u8("йНОХПНБЮРЭ НРВ╦Р"),
    no_cache     = u8("мЕР ДЮММШУ ≈ НРЙПНИРЕ ДХЮКНЦ ОПНДЮФХ ПЮМЦЮ"),
}
...
imgui.OnFrame(function() return show_menu[0] end, function()
    ...
    if imgui.Begin("###RTMain", show_menu, flags) then
        ...
        if imgui.BeginChild("##hdr", imgui.ImVec2(0, hdr_h), true) then
            ...
            if has_cache then
                imgui.RightText(u8(" цнрнбн"), math.floor(10*DPI), C.t_green)
            else
                imgui.RightText(u8(" нфхдюмхе"), math.floor(10*DPI), C.t_gray)
            end
            imgui.EndChild()
        end
        ...
        if imgui.BeginTabBar("##tabs") then
            ...
            if imgui.BeginTabItem(L.tab_main) then
                ...
                if imgui.BeginChild("##card", imgui.ImVec2(0, S.card_h), true) then
                    ...
                    if has_cache then
                        ...
                        imgui.Separator()
                        imgui.Spacing()
                        local report = u8("оНЙСОЮРЕКЭ:")..toUI(Cache.buyer).." | "..os.date("%d.%m.%Y").." | "..fmtMoney(Cache.price)
                        imgui.TextColored(C.t_gray, u8("дКЪ НРВ╦РЮ:"))
                        imgui.Spacing()
                        ...
                        if imgui.Button(L.btn_copy.."##cp", imgui.ImVec2(-1, S.btn_h)) then
                            ...
                            sampAddChatMessage("{2ecc71}[RankTracker] "..u8("яЙНОХПНБЮМН!"), -1)
                        end
                        imgui.PopStyleColor(3)
                    else
                        imgui.Spacing()
                ...
                if has_cache then
                    ...
                    if imgui.Button(L.btn_send, imgui.ImVec2(-1, S.btn_h)) then
                        sendLog(Cache.buyer,Cache.rank,Cache.days,Cache.price,Cache.profit,u8("псвмюъ нропюбйю"))
                    end
                    imgui.PopStyleColor(3)
                    imgui.Spacing()
                end
            ...
            if imgui.BeginTabItem(L.tab_stats) then
                imgui.Spacing()
                imgui.CenterText(u8("яРЮРХЯРХЙЮ ЯЕЯЯХХ"), C.t_title)
                imgui.Spacing()
                imgui.Separator()
                imgui.Spacing()
                ...
                if imgui.BeginChild("##sc", imgui.ImVec2(0, math.floor(200*DPI)), true) then
                    ...
                    sr(u8("бЯЕЦН ЯДЕКНЙ:"),      tostring(Stats.total_sales),  C.t_buyer)
                    sr(u8("оНЙСОНЙ:"),            tostring(Stats.purchases),    C.t_price)
                    sr(u8("оПНДКЕМХИ:"),          tostring(Stats.renewals),     C.t_log_ext)
                    imgui.Spacing(); imgui.Separator(); imgui.Spacing()
                    sr(u8("нАЫЮЪ ЯСЛЛЮ ОПНДЮФ:"), fmtMoney(Stats.total_income), C.t_rank)
                    sr(u8("нАЫХИ ДНУНД (50%):"),  fmtMoney(Stats.total_profit), C.t_profit)
                    if Stats.total_sales > 0 then
                        imgui.Spacing(); imgui.Separator(); imgui.Spacing()
                        sr(u8("яПЕДМЪЪ ЯДЕКЙЮ:"), fmtMoney(math.floor(Stats.total_income/Stats.total_sales)), C.t_days)
                    end
                    imgui.Spacing()
                    imgui.EndChild()
                end
                ...
                if imgui.Button(u8("яАПНЯХРЭ ЯРЮРХЯРХЙС"), imgui.ImVec2(-1, S.btn_h)) then
            ...
            if imgui.BeginTabItem(L.tab_settings) then
                ...
                imgui.TextColored(C.t_label, u8("ьХПХМЮ НЙМЮ:"))
                imgui.SameLine(math.floor(160*DPI))
                ...
                imgui.TextColored(C.t_label, u8("бШЯНРЮ НЙМЮ:"))
                imgui.SameLine(math.floor(160*DPI))
                ...
                imgui.PushStyleColor(imgui.Col.Button,        imgui.ImVec4(0.09,0.50,0.26,0.90))
                imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(0.12,0.65,0.33,1.00))
                if imgui.Button(u8("яНУПЮМХРЭ ПЮГЛЕП##sz"), imgui.ImVec2(-1, S.btn_h)) then
                ...
                imgui.Spacing()
                local cmds = {
                    {"/fmenu",   u8("нРЙПШРЭ/ГЮЙПШРЭ ЛЕМЧ")},
                    {"/rtest",   u8("рЕЯРНБЮЪ НРОПЮБЙЮ Б Discord")},
                    {"/rname X", u8("сЯРЮМНБХРЭ ХЛЪ ЛЕМЕДФЕПЮ")},
                    {"/rupdate", u8("нАМНБХРЭ ЯЙПХОР")},
                }
...
function main()
    ...
    loadLogsFromFile()
    sampAddChatMessage("{43b581}[RankTracker v3.3] Started! Manager: "..getManagerName(), -1)
